...
function StatusIndicator({ item }: { item: GradingItem }) {
  const fields = [
    { name: '实际是否正确', value: item.actual_is_correct },
    { name: '实际题目类型', value: item.actual_question_type },
    { name: '解析可接受度', value: item.analysis_acceptability },
  ];
  ...
  return (
    <div
      className={`rounded-lg p-3 cursor-pointer transition-all duration-300 border-2 relative h-32 overflow-hidden
        ${item.is_correct ? 'border-l-4 border-green-500 shadow-green-100' : 'border-l-4 border-red-500 shadow-red-100'}
  ...
  return (
    <div className="h-full bg-white rounded-lg shadow-sm p-4 overflow-y-auto">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {data.grading_report.map((item, index) => (
          <GradingCard
            key={index}
            item={item}
            index={index}
